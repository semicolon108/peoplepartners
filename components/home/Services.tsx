...
import { Briefcase, Building, Settings, Search, TrendingUp, BookOpen, TramFront, BarChart3 } from 'lucide-react';
...
const servicesData = [
    ...
    { icon: <BarChart3 size={28} />, title: 'Salary Survey', desc: 'Transform Your HR Strategy with Data-Driven Salary Intelligence- comprehensive salary survey data that takes the guesswork out of compensation planning.', href: '/services/salary-survey' },
...
export default function Services() {
    return (
        <section id="services" className="py-20 bg-brand-gray-50">
            <div className="container">
                <h2 className="text-3xl md:text-4xl font-bold text-center mb-12 text-brand-blue-900">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {servicesData.map((service) => (
                        <div key={service.title} className="bg-white p-8 rounded-2xl shadow-md hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 border border-slate-100 flex flex-col">
                            <div className="w-16 h-16 bg-gradient-to-br from-brand-blue-700 to-brand-blue-400 text-white rounded-xl flex items-center justify-center mb-6">
                                {service.icon}
                            </div>
                            <h3 className="text-xl font-semibold mb-3 text-brand-blue-700">{service.title}</h3>
                            <p className="text-brand-gray-500 mb-6 flex-grow">{service.desc}</p>
                            <Link href={service.href} className="text-brand-blue-500 font-semibold hover:underline mt-auto self-start">
                                Learn more →
                            </Link>
                        </div>
                    ))}
